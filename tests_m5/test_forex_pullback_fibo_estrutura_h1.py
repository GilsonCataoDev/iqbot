...
def test_futuro_nao_muda_sinais_passados() -> None:
    candles = _candles()
    antes = planos_pullback_fibo_estrutura_h1("EURUSD", candles.iloc[:350])
    alterado = candles.copy()
    colunas = alterado.columns.get_indexer(["Open", "High", "Low", "Close"])
    alterado.iloc[350:, colunas] *= 1.5
    depois = planos_pullback_fibo_estrutura_h1("EURUSD", alterado).iloc[:350]
...
def test_backtest_aceita_estrategia() -> None:
    resultados, banca = simular_forex(
        "EURUSD", _candles(), estrategia="pullback_fibo_estrutura_h1", spread=0.00012
    )
    assert isinstance(resultados, pd.DataFrame)
    assert banca > 0


def test_filtro_fibo_nao_usa_futuro() -> None:
    candles = _candles()
    antes = planos_pullback_h1_com_fibo("EURUSD", candles.iloc[:350])
    alterado = candles.copy()
    colunas = alterado.columns.get_indexer(["Open", "High", "Low", "Close"])
    alterado.iloc[350:, colunas] *= 1.5
    depois = planos_pullback_h1_com_fibo("EURUSD", alterado).iloc[:350]
    assert [_chave(x) for x in antes] == [_chave(x) for x in depois]


def test_backtest_aceita_pullback_com_fibo() -> None:
    resultados, banca = simular_forex(
        "EURUSD", _candles(), estrategia="pullback_h1_com_fibo", spread=0.00012
    )
    assert isinstance(resultados, pd.DataFrame)
    assert banca > 0
